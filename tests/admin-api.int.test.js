...
describe('/api/admin', () => {
  ...
  // ======================= Create root admin ========================
  // ===============================================================
  describe('POST /create-root-admin', () => {
    const exec = value => {
      return request(server)
        .post('/api/admin/create-root-admin')
        .send(value);
    };
    ...
    it('should redirect if root admin already exist', async () => {
      expect.assertions(1);

      await Admin.create(dataBaseData);
    ...
    it('should response with 201 if all data is valid', async () => {
      ...
      expect(body).toHaveProperty(
        'message',
        'Root Admin created successfully. Please verify your email'
      );
    });
  });
  ...
  describe('POST /create-admin', () => {
    ...
    it('should return 403 if admin has not permission', async () => {
      expect.assertions(2);
    ...
    it('should return 406 validation error if data is invalid', async () => {
      expect.assertions(2);
    ...
    it('should return 406 error if email is already exist', async () => {
      expect.assertions(2);
    ...
    it('should create an admin and send verification email if all data is valid', async () => {
      expect.hasAssertions();
  ...
  describe('GET /resend-activation-link/:adminId', () => {
    ...
    it('should return 406 if user already activated', async () => {
      ...
      const { status, body } = await exec(admin._id);
    ...
    it("should return 200 and send new activation email if user isn't activated", async () => {
      ...
      const admin = await Admin.create(dataBaseData);
      const { status, body } = await exec(admin._id);
  ...
  describe('GET /active-account', () => {
    ...
    it('should return 400 if account already activated', async () => {
      ...
      const admin = await Admin.create({ ...dataBaseData, isActivated: true });
      const token = await tokenGenerator({
        _id: admin._id
      });
  ...
  describe('POST /login', () => {
    ...
    it('should return 403 if admin account is disabled', async () => {
      ...
      await Admin.create({ ...dataBaseData, isDisable: true });
    ...
    it('should return 401 if password do not match', async () => {
      ...
      await Admin.create({ ...dataBaseData, isActivated: true });
  ...
  describe('PUT /change-role/:adminId', () => {
    ...
    it('should change admin role', async () => {
      ...
      const { status, body } = await exec({ role: 'BASIC' }, admin._id).set(
        'Cookie',
        [`x-access-token=${token}`]
      );
